...
<div class="container">
    <div class="row justify-content-center" style="margin-left: -400px; margin-right:-400px ">
        <div class="col-md-24">
            <div class="card">
                <div class="card-header text-center" ><h1>Liste des Adhesions
...
<th><input type="number" readonly value="{{$project->renumeration}}" style="color:black"  name="salaire" class="form-control mb-2 mr-sm-2"></th>
         ...
         @if($project->statut==0)
            <th><input type="text"  style="color:black"  readonly value="En Attente ..."  name="statut" class="form-control mb-4 mr-sm-4"></th> 
           @endif   
            @if($project->statut==1)
            <th><input type="text"  style="color:black"  readonly value="Enrégistrer"  name="statut" class="form-control mb-4 mr-sm-4"></th> 
           @endif   
<th><a  href="{{route('valider_demande',['value'=>$project->id])}}" class="btn btn-lg btn-success btn-block " onclick="return window.confirm('voulez-vous valider cette adhésion ?')" >Autoriser </a></th>
<th><a  href="{{route('annuler_demande',['value'=>$project->id])}}" class="btn btn-lg btn-danger btn-block " onclick="return window.confirm('voulez-vous refuser cette adhésion ?')" >Rejeter </a></th>
</tr>
